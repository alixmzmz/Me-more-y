...
# ------------------------------------------------------------------------------------------------------------------------------------------------
# FUNCTION to calculate Maximum HEIGHT per ROW

def calculateAllHeights(images, width):
    ...
    while images_list:
        # get first image
        img_path = images_list.pop(0) # pop = extract first array element
        # open image
        img = Image.open(img_path)

        # PIL http://pillow.readthedocs.io/en/3.1.x/reference/Image.html
        # ARRAYS in python = lists + tuple - https://www.youtube.com/watch?v=kTsahM462aQ
        if (img.height > maxRowHeight):
            maxRowHeight = img.height

        # when 'x' will go beyond the 'width', start again
        if (x > width):
            print('Number of line: %d height: %d' % (numberOfline, maxRowHeight))
            row_heights.append(maxRowHeight) # include the highest imageRowHeight in the arrray row_heights
            maxRowHeight= 0
            x = 0
            numberOfline += 1
        x += img.width + margin_size

    # Comments on python - https://learnpythonthehardway.org/book/ex5.html - https://stackoverflow.com/questions/4288973/whats-the-difference-between-s-and-d-in-python-string-formatting
    print('Number of line: %d ' % numberOfline)
...
def make_collage(images, filename, width, init_height):
    ...
    while True:
        ...
        while images_list:
            ...
            if x > width:
                ...
                images_line = []
                x = 0
            x += img.size[0] + margin_size # img.size[0] = full output image width
    ...
    for coef, imgs_line in coefs_lines:
        if imgs_line:
            ...
            if(row_heights): # if there are elements in the array do the following
                currentRowHeight = row_heights.pop(0) # extract the first value of the row_heights array
                y += currentRowHeight + margin_size # add to y the height of the current row
                numberOfline += 1
...
def main():
    ...
    calculateAllHeights(images, args.width)
