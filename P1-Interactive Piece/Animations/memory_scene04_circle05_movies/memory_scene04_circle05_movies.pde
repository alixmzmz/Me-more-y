...
void setup() {
  ...
  for (int x=0; x <= width; x+=random(70, 150)) {
    for (int y=0; y <= height; y+=random(70, 150)) {
      PImage img; 
      imageNumber = int(random(1, 226)); // 50 number of final book images on the folder
      img = loadImage("data/images/movies_" + imageNumber +".jpg");
...
void draw() {
  ...
  //saveFrame("exportVideo/videoMovies-######.png");
}