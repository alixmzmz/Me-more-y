...
void setup() {
  ...
  for (int x=0; x <= width; x+=random(70, 150)) {
    for (int y=0; y <= height; y+=random(70, 150)) {
      PImage img; 
      imageNumber = int(random(1, 242)); // 242 number of final history images on the folder
      img = loadImage("data/images/history_" + imageNumber +".jpg");
...
void draw() {
  ...
  if (countFrames == 10) { // 2-counter - 14 frames will load an image
    for (int x=0; x <= width; x+=random(400)) {
      ...
      imageNumber = int(random(1, 242)); // 50 number of final book images on the folder
      img = loadImage("data/images/history_" + imageNumber +".jpg");
  ...
  //PImage imageFrame = loadImage("data/frame.jpg");
  //imageFrame.resize(1680,1050);
  //maskImageGrey = loadImage("data/masks/mask_grey.png");
  //imageFrame.mask(maskImageGrey);
