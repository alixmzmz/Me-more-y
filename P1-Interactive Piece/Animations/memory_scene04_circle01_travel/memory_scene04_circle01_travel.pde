...
void setup() {
  ...
  for (int x=0; x <= width; x+=random(70, 150)) {
    for (int y=0; y <= height; y+=random(70, 150)) {
      PImage img; 
      travelImageNumber = int(random(1, 413)); // 50 number of final travel images on the folder
      img = loadImage("data/images/travel_" + travelImageNumber +".jpg");
...
void draw() {
  ...
  if (countFrames == 10) { // 2-counter - 14 frames will load an image
    for (int x=0; x <= width; x+=random(400)) {
      ...
      travelImageNumber = int(random(1, 413)); // 413 number of final travel images on the folder
      img = loadImage("data/images/travel_" + travelImageNumber +".jpg");
    
      if (img != null) {
        img.resize(img.width/3, img.height/3); // travel image resized to height
  ...
  //saveFrame("exportVideo/videoTravel-######.png");
}